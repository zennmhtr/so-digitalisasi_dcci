...
// 1. Add positions helper at the top of the component
if (!content.includes('const getPos = ')) {
  const helper = `
  const getPos = (key, defaultX, defaultY, widthClass = 'w-44') => {
    const pos = organizationData?.positions?.[key];
    return {
      style: { position: 'absolute', left: pos?.x || defaultX, top: pos?.y || defaultY, zIndex: 20 },
      className: widthClass
    };
  };
  `;
  content = content.replace('const handlePrint = () => {', helper + '\n  const handlePrint = () => {');
}

// 2. Remove grid wrappers
content = content.replace(/<div className="grid grid-cols-6 gap-4">/g, '');
content = content.replace(/<div className="flex flex-col gap-0">/g, '');
content = content.replace(/<div className="space-y-4">/g, '');
content = content.replace(/<div className="space-y-3">/g, '');
content = content.replace(/<div className="min-h-\[\d+px\]"><\/div>/g, '');
content = content.replace(/<div className="flex justify-center items-center" style={{ height: "20px" }}>[\s\S]*?<\/div>\s*<\/div>/g, '');

// 3. Set minimum height for gridRef wrapper
...
// 4. Update refs with absolute styles
const replacements = [
  // bod
...
replacements.forEach(({ ref, key, x, y, w }) => {
  const regex = new RegExp('<div ref={' + ref + '} className="bg-(white|gray-200) (.*?)"(.*?)>', 'g');
  content = content.replace(regex, (match, bg, classes, rest) => {
    return `<div ref={${ref}} style={getPos('${key}', ${x}, ${y}, '${w}').style} className={\`bg-${bg} ${classes} \${getPos('${key}', ${x}, ${y}, '${w}').className}\`}${rest}>`;
  });
});
